...
def preprocess_image(image_path, fixed_size, device):
...
def inference(model, image_path, fixed_size, device):
    """Perform inference on a single image."""
    image, original_size = preprocess_image(image_path, fixed_size, device)
    with torch.no_grad():
        output = model(image)
    pred = postprocess_output(output, original_size)
    return pred
...
if __name__ == "__main__":
    ...
    test_dir = "//home/william/extdisk/data/feynman/FC1/GY2ASH24GV0019/L/"
    pred_dir = "/home/william/extdisk/data/feynman/FC1/GY2ASH24GV0019/L/predictions"
    ...
    for p in tqdm(image_paths):
        ...
        pred = inference(model, image_path, fixed_size, device)
